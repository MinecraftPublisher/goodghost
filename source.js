...
function clean(text, message) {
  if (filterr.clean(text) == text) {
    //Clean.Noice.
    return text;
  } else {
    //Some words were filtered
    return "null";
  }
}
client.on("ready", () => {
    ...
    guild.channels.cache
      ...
      .send(
        "Sup guyz,i am Good ghost. 👋 \nI am a chatbot 🤖 ,but you can count me as a user. 👨🏻\nSo i can chat with you in activated channels.\nI am currently in **" +
          ...
          "\nUsing 'gg|' at the first of your message will cause me to ignore u.\nGood ghost's discord: https://discord.gg/wXBQyD2"
      );
    guild.channels.cache
  ...
  if (msg.channel.type === "dm") {
    ...
    if (msg.author.bot) {
      ...
      if (
        ...
        clean(msg.content.toLowerCase(), msg).indexOf("gg|") > -1
      ) {
        return;
      }
      //Blockin' out people?
        ...
        if (fs.existsSync("activates/" + msg.channel.id)) {
          ...
          //Doing keepup
            ...
            msg.channel.send(
              "Due to the hosting restrictions of Good ghost,\nGood ghost would turn off once in a while.\nYou can support it by clicking on this link and waking up Good ghost.\nhttps://tya-discord-bot.glitch.me\n**Note:**```If you have an iOS device,either iphone or ipad,you can add an app to wake up good ghost easily.use this link to add good ghost to your homescreen\nTo add,click on the link.then click allow and then close.\nthen,navigate to your settings app,general,profiles/profiles and device management and then click on good ghost.\nclick install,enter your password and click done.now good ghost app must be added to your homescreen.```http://tya.martia.net/gg.mobileconfig"
            );
          }
          //Pingggggggggg
          ...
          //fs.writeFile("banz/" + msg.mentions.users.first(), "Banned", function() {
          //Gotta like unban
...
client.on("message", msg => {
  ...
  setTimeout(respond, n * 100);
  setTimeout(untype, n * 100);
});
